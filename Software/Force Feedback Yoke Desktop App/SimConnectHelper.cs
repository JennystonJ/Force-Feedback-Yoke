...
namespace Force_Feedback_Yoke_Desktop_App
{
    ...
    public struct AircraftData
    {
        ...
        public double engineRPM;           // rpm
        public double elevatorTrim;        // radians
    ...
    public class SimPauseEventArgs : EventArgs
    {
        public bool Paused { get; internal set; }
        public SimPauseEventArgs(bool paused)
        {
            Paused = paused;
        }
    }
    ...
    public class SimConnectHelper : Control
    {
        ...
        private void SimConnect_OnRecvEvent(SimConnect sender, SIMCONNECT_RECV_EVENT data)
        {
            if (sender != null)
            {
                ...
                {
                    case EVENT_ID.PAUSE:
                        bool paused = data.dwData == 1;
                        SimPauseChangedEvent?.Invoke(this, new SimPauseEventArgs(paused));
                        break;
                    default:
                        break;
                }
        ...
        private void RegisterSimVar()
        {
            if(simConnect != null)
            {
                try
                {
                    ...
                    simConnect.AddToDataDefinition(DEFINITION.ControlAxisData, "ELEVATOR POSITION", "position 16k",
                        SIMCONNECT_DATATYPE.FLOAT64, 0.0f, SimConnect.SIMCONNECT_UNUSED);
                    simConnect.AddToDataDefinition(DEFINITION.ControlAxisData, "AILERON POSITION", "position 16k",
                       SIMCONNECT_DATATYPE.FLOAT64, 0.0f, SimConnect.SIMCONNECT_UNUSED);
        ...
        private void RegisterAircraftSimVar()
        {
            if(simConnect != null)
            {
                try
                {
                    ...
                    simConnect.AddToDataDefinition(DEFINITION.AircraftData, "ELEVATOR TRIM POSITION", "radians",
                        SIMCONNECT_DATATYPE.FLOAT64, 0.0f, SimConnect.SIMCONNECT_UNUSED);
        ...
        public void SetValue(ControlAxisData structSimVarOut)
        {
            if (simConnect != null)
            {
                if (InputEnabled)
                {
                    simConnect.SetDataOnSimObject(DEFINITION.ControlAxisData, SimConnect.SIMCONNECT_OBJECT_ID_USER,
