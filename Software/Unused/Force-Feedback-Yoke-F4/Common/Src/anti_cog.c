...
//	antiCog->hardware.motor = NULL;
	...
	LowPassFilterInit(&antiCog->calibration.lpfError, 0.1f);
	LowPassFilterInit(&antiCog->calibration.lpfDerivative, 0.1);

	PID_t *pid = &antiCog->calibration.pidPosition;
	PIDInit(pid);
	PIDSetGains(pid, 0.06050f, 0.00002f, 0.00028f);
	PIDSetIntegralLimit(pid, 4000000);
	...
	PIDInit(pidSpeed);
	PIDSetGains(pidSpeed, 10.0f, 0.022000f, 0);
	PIDSetIntegralLimit(pidSpeed, 5000);
	pidSpeed->integralEpsilon = 0.5f;



	LowPassFilterInit(&antiCog->calibration.lpfOutput, 0.1f);

	antiCog->calibration.velocityDampingCo = 0.1f;
	LowPassFilterInit(&antiCog->calibration.lpfSpeed, 0.005f);
	...
	antiCog->cogMapStepsPerCount =
			...
			EncoderGetCountPerRev(antiCog->hardware.encoder);
}
...
float AntiCogCalc(AntiCog_t *antiCog) {
	...
	float encoderCount = EncoderGetHardCount(antiCog->hardware.encoder);
