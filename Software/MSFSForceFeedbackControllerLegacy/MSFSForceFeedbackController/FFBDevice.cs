...
namespace MSFSForceFeedbackController
{
    public class FFBDevice
    {
        ...
        public HIDEvent FFBHidDataReadyCallback;
        ...
        private byte[] outBufferBack;
        private byte[] outBufferA = new byte[outBufferLength + 1];
        private byte[] outBufferB = new byte[outBufferLength + 1];
        ...
        {
	        internal bool capabilityInitialized;
            private float[] axisTravelRanges = new float[FFB_HID_NUM_AXIS];
            private float[] axisMaxPower = new float[FFB_HID_NUM_AXIS];
            private byte maxSupportedForces;

            public CapabilityData()
            {
                capabilityInitialized = false;
                Array.Clear(axisTravelRanges);
                Array.Clear(axisMaxPower);
                maxSupportedForces = 0;
            }

            public float[] AxisTravelRanges 
            { 
                get { return axisTravelRanges; } 
                internal set { axisTravelRanges = value; } 
            }
            public float[] AxisMaxPower
            {
                get { return axisMaxPower; }
                internal set { axisMaxPower = value; }
            }
            public byte MaxSupportedForces {  
                get { return maxSupportedForces; } 
                internal set { maxSupportedForces = value; }
            }
        }
        ...
        public ForceReport forceReport { get; set; }
        ...
        public double GetMeasuredPitchForce()
        {
            if (capabilities.capabilityInitialized &&
                capabilities.AxisMaxPower[FFB_HID_PITCH_INDEX] > 0)
            {
                return Utilities.Scale(hidData.pitchForce, -32767, 32767,
                    -capabilities.AxisMaxPower[FFB_HID_PITCH_INDEX],
                    capabilities.AxisMaxPower[FFB_HID_PITCH_INDEX]);
            }
            else
            {
        ...
        {
            if (capabilities.capabilityInitialized &&
                  capabilities.AxisMaxPower[FFB_HID_ROLL_INDEX] > 0)
            {
                return Utilities.Scale(hidData.rollForce, -32767, 32767,
                    -capabilities.AxisMaxPower[FFB_HID_ROLL_INDEX],
                    capabilities.AxisMaxPower[FFB_HID_ROLL_INDEX]);
            }
            else
            {
        ...
        private async Task<int> StartHid(Task<ConnectStatus> hidTask, TimerCallback callback)
        {
            ...
            outBufferA = new byte[19];
            outBufferA[0] = HID_OUT_REPORT_ID;
            RefreshCapabilities();
            _ = HidWorkerLoop();
            _ = HidReaderLoop();
        ...
        byte[] PrepareFFBReport(ForceSet force)
        {
            ...
            float maxForce = capabilities.AxisMaxPower[FFB_HID_PITCH_INDEX];
            ...
            //Array.Copy(aileronForceOutBytes, 0, outBufferA, 1, aileronForceOutBytes.Length);
            //Array.Copy(elevatorForceOutBytes, 0, outBufferA, aileronForceOutBytes.Length + 1,
            //    elevatorForceOutBytes.Length);
            //outBufferA[0] = HID_OUT_REPORT_ID; // reportId byte (force data)
        }
        ...
        private async Task HidReaderLoop()
        {
            while (true) 
            { 
                ...
                try
                {
                    byte[] hidBuffer = await device.ReadAsync();
        ...
        private async Task HidWorkerLoop()
        {
            while (true)
            {
                    ...
                    try
                    {
                        switch(command[0])
                        {
                            ...
                            case HID_OUT_REPORT_ID:
                                ...
                                //byte[] hidBuffer = await TransferHid(command);       
                                //ProcessHID(hidBuffer);
                                //hidReceived = true;
                                break;
                            case HID_CAPABILITIES_REPORT_ID:
        ...
        private void ProcessCapabilities(byte[] data)
        {
            ...
            capabilities.AxisTravelRanges[FFB_HID_PITCH_INDEX] = BitConverter.ToSingle(values, 1);
            capabilities.AxisTravelRanges[FFB_HID_ROLL_INDEX] = BitConverter.ToSingle(values, 5);
            capabilities.AxisMaxPower[FFB_HID_PITCH_INDEX] = BitConverter.ToSingle(values, 9);
            capabilities.AxisMaxPower[FFB_HID_ROLL_INDEX] = BitConverter.ToSingle(values, 13);
            capabilities.MaxSupportedForces = values[17];

            //if (capabilities.capabilityInitialized && forces != null)
            //{
            //    //forces = new HardwareForce[capabilities.MaxSupportedForces];
            //}
        }
